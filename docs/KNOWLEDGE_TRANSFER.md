...
> 2026-04-29 update (marketplace Phase 2 – recruiter invites): Added API endpoints to support multi-user recruiter teams: `GET/POST /api/recruiter/company/invites` (list/create invite tokens) and `POST /api/recruiter/company/invites/accept` (accept invite + create membership). Email sending is best-effort when enabled (see Phase 6 notes) and the API also returns an invite URL token.
...
> 2026-05-02 update (E2E mock auth parity): `GET/PATCH /api/daily-actions`, `GET/PATCH /api/opportunity-alerts`, `GET/POST /api/streak`, and `GET/POST /api/streak-rewards` now authenticate via `getUser()` from `lib/auth.ts` (same as other routes) so Playwright cookie mock auth (`e2e-mock-role` / `e2e-mock-secret`) applies instead of only `supabase.auth.getUser()`.
> 2026-05-02 update (full-site inventory coverage): Added **`docs/QA_FULL_SITE_COVERAGE.md`** (defines Layer A/B/C: inventory smoke vs behavioral vs deterministic AI). Added **`docs/QA_CHANGE_CHECKLIST.md`** (what to update when routes/tests/docs change). Added **`api-tests/inventory-mutations-smoke.api.spec.ts`** (POST/PATCH/DELETE smoke for all inventory APIs), **`e2e/full-site-page-inventory-mock.spec.ts`** (all 66 pages with mock auth + placeholder dynamic segments), **`e2e/helpers/ai-deterministic-mocks.ts`** (optional stable AI POST mocks), npm script **`test:e2e:full-inventory-pages`**. **`GET /api/career-coach`** and **`POST /api/opportunity-alerts/scan`** now use `getUser()` for mock-auth parity.
> 2026-05-02 update (middleware + Supabase browser client): **`middleware.ts`** treats **`/api/internal/e2e-*`** like other non-session API exceptions so Playwright can call dev-only bootstrap routes without an existing session (handlers still validate secrets / disable in production). **`lib/supabase/client.ts`** sets cookie `secure` only in production so auth cookies work on localhost.
> 2026-05-02 update (mobile PWA OAuth / localhost): **`lib/appUrl.ts`** exposes **`getOAuthRedirectOrigin()`** (uses **`NEXT_PUBLIC_APP_URL`** when set, else `window.location.origin`) for Supabase OAuth and email redirects from **`app/login`**, **`app/signup`**, **`app/login/reset`**. Avoids **`ERR_CONNECTION_REFUSED`** on phones when a homescreen PWA was installed from **`http://localhost:3000`**. **`getAppBaseUrl()`** lives in **`lib/appUrl.server.ts`** (uses **`next/headers`**) so API routes can build absolute links without pulling server APIs into client bundles.
> 2026-05-02 update (job applications API): **`GET /api/job-applications`** loads `job_postings` joined fields then resolves **`companies`** in a second query by `company_id` (same response shape: each row still includes optional **`company`** for the UI).

## 2026-05-01 update (QA plan: Playwright site-wide + real-user mode)

- Added `docs/QA_SITE_WIDE_PLAYWRIGHT_PLAN.md` describing a two-mode QA approach:
  - **Deterministic CI mode** using existing cookie-based mock auth E2E + API contract tests (`npm run test:all:qa`)
  - **Real-user regression mode** using Playwright session reuse (`playwright/.auth/*.json`) for both roles and mobile+desktop coverage; credentials stay in `.env.e2e.local` (gitignored).
- Added real-user Playwright scaffolding: `e2e/auth/real-login.setup.ts`, `e2e/real/*.real.spec.ts`, plus `playwright.config.ts` projects (`real-setup`, `real-jobseeker-*`, `real-recruiter-*`).
- Added `.env.e2e.example` template and npm scripts under `package.json` to run real-user projects locally.
- **Playwright base URL:** default is `http://localhost:<PLAYWRIGHT_PORT>` (not `127.0.0.1`) so Supabase auth cookies line up with the dev origin; override with `PLAYWRIGHT_BASE_URL` if needed.
- Real-user smoke specs assert **URL pathname** (not only headings): login and dashboard both expose “Welcome back”, so `/dashboard`/`/resume-analyzer` checks prevent false positives.
- Setup saves enriched storage state (CDP cookies + full `localStorage` for the app origin) because bare `storageState()` could serialize empty for `@supabase/ssr` chunked cookies.
- Added deterministic **site-wide page sweep** `e2e/site-wide-sweep.spec.ts` (mock auth): visits **public + jobseeker + recruiter** routes, asserts the shell renders, checks no horizontal overflow, and fails on real console errors. Uses minimal endpoint-specific API fixtures (e.g. `/api/career-coach`, `/api/usage`, `/api/resume-performance`, `/api/skill-demand`) plus a small navigation retry helper to keep CI runs stable without relying on real data.
- Added real-user **dynamic public pages** regression `e2e/real/public-dynamic.real.spec.ts` + Playwright project `real-public-dynamic-desktop`: validates dynamic pages that read from Supabase in server components (e.g. `/jobs/[slug]`, `/results/[token]`, `/u/[slug]`, `/salary/[slug]`). These cannot be fully stabilized with client-side `/api/**` mocks; they are best covered in real-user mode.
- Added real-user **messaging regression** `e2e/real/messages.real.spec.ts` + Playwright project `real-messages-desktop`: recruiter sends a message to the jobseeker (API-assisted), and both `/recruiter/messages` + `/messages` UIs show the new content.
- Added real-user **recruiter candidates regression** `e2e/real/recruiter-candidates.real.spec.ts` + Playwright project `real-recruiter-candidates-desktop`: finds a real candidate via `/api/recruiter/candidates`, opens `/recruiter/candidates/[id]`, and best-effort triggers ATS analysis (skips gracefully if no candidates/resume/credits).
- Added real-user **recruiter job AI regression** `e2e/real/recruiter-job-ai.real.spec.ts` + Playwright project `real-recruiter-job-ai-desktop`: creates/reuses a job, runs `/recruiter/jobs/[id]/optimize` (best-effort for credits), and runs `/recruiter/jobs/[id]/auto-shortlist` (best-effort for credits/no applications).
- Added real-user **jobseeker auto/smart-apply regression** `e2e/real/jobseeker-auto-smart-apply.real.spec.ts` + Playwright project `real-jobseeker-auto-smart-apply-desktop`: uses an existing resume to (best-effort) start an Auto-Apply run (`POST /api/auto-apply`) and create/toggle a Smart Auto-Apply rule (`POST/PATCH /api/smart-apply`), with graceful skips when plan limits block these Pro features.
- Added real-user **cover-letter + improve-resume regression** `e2e/real/jobseeker-cover-letter-improve.real.spec.ts` + Playwright project `real-jobseeker-cover-letter-improve-desktop`: generates a cover letter (`POST /api/generate-cover-letter`) and verifies it appears in `/history`, then improves a resume (`POST /api/improve-resume`) and verifies an improved resume entry appears in `/history` (best-effort for plan/credit limits).
- Added real-user **job match + resume tailoring regression** `e2e/real/job-match-tailor.real.spec.ts` + Playwright project `real-jobseeker-jobmatch-tailor-desktop`: runs `POST /api/job-match` and `POST /api/improve-resume` (tailor mode) best-effort for limits/credits, then verifies `/job-match` and `/tailor-resume` pages render for the session.
- Added real-user **LinkedIn import regression** `e2e/real/linkedin-import.real.spec.ts` + Playwright project `real-jobseeker-linkedin-import-desktop`: verifies `/import-linkedin` loads and `POST /api/import-linkedin` accepts a minimal sample profile text (best-effort for credit exhaustion).
- Added real-user **onboarding regression** `e2e/real/onboarding.real.spec.ts` + Playwright project `real-onboarding-desktop`: jobseeker covers `/onboarding` (upload TXT → ATS score screen), recruiter covers `/recruiter/onboarding` (create company if needed, or accept redirect when company already exists).
- Added real-user **site-wide route sweeps**:
  - `e2e/real/jobseeker-sitewide.real.spec.ts` + Playwright project `real-jobseeker-sitewide-desktop`: visits major jobseeker pages (dashboard, tools, insights, settings, etc.), asserts render + no horizontal overflow + no real console errors.
  - `e2e/real/recruiter-sitewide.real.spec.ts` + Playwright project `real-recruiter-sitewide-desktop`: visits major recruiter pages (jobs, candidates, ATS, templates, alerts, company, pricing, settings, etc.), asserts render + no horizontal overflow + no real console errors.
- Added real-user **settings regression** `e2e/real/jobseeker-settings.real.spec.ts` + Playwright project `real-jobseeker-settings-desktop`: updates jobseeker display name via Settings UI and verifies preference persistence via `PATCH/GET /api/user`.
- Added real-user **recruiter templates/alerts regression** `e2e/real/recruiter-templates-alerts-invites.real.spec.ts` + Playwright project `real-recruiter-templates-alerts-invites-desktop`: creates a message template (`POST /api/recruiter/templates`) and a saved alert (`POST /api/recruiter/alerts`) and verifies both appear in the UI; invite accept is best-effort (may be blocked by plan/RLS).
- Added real-user **recruiter job CRUD regression** `e2e/real/recruiter-jobs-crud.real.spec.ts` + Playwright project `real-recruiter-jobs-crud-desktop`: creates a draft job via the UI (`/recruiter/jobs/new`) and verifies it appears via `GET /api/recruiter/jobs` and in `/recruiter/jobs` UI (best-effort toggle).
- Added real-user **recruiter job edit regression** `e2e/real/recruiter-job-edit.real.spec.ts` + Playwright project `real-recruiter-job-edit-desktop`: creates a job via `POST /api/recruiter/jobs`, edits it via `/recruiter/jobs/[id]`, and verifies the updated title via `GET /api/recruiter/jobs/[id]`.
- Added real-user **recruiter applications stage UI regression** `e2e/real/recruiter-applications-ui-stage.real.spec.ts` + Playwright project `real-recruiter-applications-ui-stage-desktop`: creates a job, applies as jobseeker, then updates the application stage to `shortlisted` via the recruiter `/recruiter/applications` UI (list view).
- Added `/recruiter/invite/accept` page (`app/(recruiter)/recruiter/invite/accept/page.tsx`) to handle invite URLs returned by `POST /api/recruiter/company/invites` (accepts `?token=` and calls `POST /api/recruiter/company/invites/accept`).
- Added real-user **recruiter invite accept UI regression** `e2e/real/recruiter-invite-accept-ui.real.spec.ts` + Playwright project `real-recruiter-invite-accept-ui-desktop`: creates an invite and verifies the accept page resolves to either success or a best-effort failure state.
- Added real-user **Quick Resume Builder regression** `e2e/real/jobseeker-resume-builder.real.spec.ts` + Playwright project `real-jobseeker-resume-builder-desktop`: completes `/resume-builder` and verifies it opens `/resume-analyzer`.
- Added real-user **History drilldown regression** `e2e/real/jobseeker-history-drilldown.real.spec.ts` + Playwright project `real-jobseeker-history-drilldown-desktop`: verifies `/history` loads and all history sections render; opens a `View` link best-effort.
- Added real-user **Messages read-sync regression** `e2e/real/jobseeker-messages-readsync.real.spec.ts` + Playwright project `real-jobseeker-messages-readsync-desktop`: if there are unread messages (via `GET /api/messages/unread-summary`), opens `/messages?peer=<id>` and asserts unread count clears after `POST /api/messages/mark-read` (best-effort when there are no unread messages).
- Added `docs/QA_PROD_CRITICAL_USER_STORIES.md` defining a finite checklist of **production-critical user stories** and mapping each to at least one **real-user action+assert** Playwright spec/project.
- Added inventory-driven static route render regression `e2e/real/route-inventory-static-render.real.spec.ts` + Playwright project `real-route-inventory-static-render-desktop`: parses `docs/QA_ROUTE_INVENTORY.md` and visits **every non-dynamic page route** under the correct auth context (public/jobseeker/recruiter), failing on error UI.
- Added **API inventory GET smoke** `api-tests/inventory-get-smoke.api.spec.ts`: cookie mock-auth probes most inventory `GET /api/**` routes (job seeker, recruiter, and two public reads); asserts HTTP responses are in a bounded set and JSON bodies parse when `Content-Type` is JSON (does not assert business/data correctness). Extended **POST validation contracts** in `api-tests/contracts.api.spec.ts` for `POST /api/auto-apply` and `POST /api/generate-cover-letter` invalid payloads.
- Added deterministic **pricing / upgrade UI branches** `e2e/pricing-branches.spec.ts`: jobseeker `/pricing` “Upgrade to Pro” (`alert` + navigate to `/settings`) and recruiter `/recruiter/pricing` Pro plan selection confirmation banner.
...
**Mobile PWA / `ERR_CONNECTION_REFUSED` to `localhost`:** After Google sign-in the browser may open **`http://localhost:3000`**. On a **phone**, `localhost` is the device itself, so the connection fails. Typical cause: the user **installed the PWA from local dev** (`http://localhost:3000`); the standalone app keeps that origin. **Mitigations:** (1) Set **`NEXT_PUBLIC_APP_URL`** to the **production** canonical URL (e.g. `https://ai-job-assistant-six-rho.vercel.app`) in Vercel env so OAuth and password-reset redirects use **`getOAuthRedirectOrigin()`** in `lib/appUrl.ts` instead of only `window.location.origin`. (2) In Supabase → URL Configuration, allow **`https://<your-app>/auth/callback`** (and `/login?reset=1` paths used by email flows). (3) Tell users to **delete the old homescreen shortcut** and **Add to Home Screen** again from the **production** site.
...
| **lib/appUrl.ts** | **`getOAuthRedirectOrigin()`** — canonical origin for Supabase OAuth / magic-link / password-reset `redirectTo` (prefers **`NEXT_PUBLIC_APP_URL`**, else `window.location.origin`). Client-safe import (no **`next/headers`**). Mitigates PWAs installed from localhost redirecting mobile devices to **`localhost`**. |
| **lib/appUrl.server.ts** | **`getAppBaseUrl()`** — async server helper for absolute URLs (`NEXT_PUBLIC_APP_URL`, then Vercel/Host headers, else `http://localhost:3000`). Used from API routes only; must not be imported by **`"use client"`** modules. |
...
| **lib/supabase/client.ts** | Browser Supabase client: `createBrowserClient` with `cookieOptions` — `secure` only in production so sessions persist on **http://localhost** during `next dev` (otherwise middleware may redirect protected routes to `/login` after full navigation). |
| **lib/auth.ts** | See §2.3. |
