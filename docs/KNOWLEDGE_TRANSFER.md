...
> 2026-05-02 update (mobile PWA OAuth / localhost): **`lib/appUrl.ts`** exposes **`getOAuthRedirectOrigin()`** (uses **`NEXT_PUBLIC_APP_URL`** when set, else `window.location.origin`) for Supabase OAuth and email redirects from **`app/login`**, **`app/signup`**, **`app/login/reset`**. Avoids **`ERR_CONNECTION_REFUSED`** on phones when a homescreen PWA was installed from **`http://localhost:3000`**. Server routes use **`getAppBaseUrl()`** for absolute invite/API links.
...
| **lib/appUrl.ts** | **`getOAuthRedirectOrigin()`** — canonical origin for Supabase OAuth / magic-link / password-reset `redirectTo` (prefers **`NEXT_PUBLIC_APP_URL`**, else `window.location.origin`). Mitigates PWAs installed from localhost redirecting mobile devices to **`localhost`**. **`getAppBaseUrl()`** — async server helper for absolute URLs (`NEXT_PUBLIC_APP_URL`, then Vercel/Host headers, else `http://localhost:3000`). |
