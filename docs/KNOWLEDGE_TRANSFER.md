...
| **RecruiterTopbar** | Recruiter header: **Messages** link (**`/recruiter/messages`**) + **NotificationBell**; optional **company logo** badge (**`useRecruiterCompany`**) next to **`UserAvatar`**. |
...
| Component | Role |
|-----------|------|
| **MessagesInbox** | Shared UI for **`/messages`** and **`/recruiter/messages`**: conversation list (**`UserAvatar`** + name from **`peer_profiles`**), thread + reply, compose; **`?compose=1&receiver_id=`** opens an existing thread when there is history (redirects to **`peer=`**); **`RecipientPicker`** (**`GET /api/messages/recipient-search`**); **`POST /api/messages/mark-read`** on thread open; attachments via **`POST /api/messages/attachment`**; **Sent/Read** on own bubbles; **`useMessagingTyping`** for peer typing indicator. |
...
- **Supabase Realtime:** Enabled on **`notifications`** for **NotificationBell**; **`public.messages`** for inbox/thread invalidation (**`useMessages`**). Typing indicators use **Realtime broadcast** (channel name **`typing:{sorted peer ids}`**); ensure Realtime is enabled for the project.
