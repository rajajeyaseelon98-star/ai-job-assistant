...
| **lib/notifications.ts** | **`createNotification`** — inserts for the **current** user (RLS). **`createNotificationForUser`** — inserts for **another** user via **`createServiceRoleClient`** when **`SUPABASE_SERVICE_ROLE_KEY`** is set (e.g. **POST /api/messages** notifies **`receiver_id`**); no-op in dev if unset. Non-critical, silently ignores errors. |
...
| **/api/messages** | GET, POST | Authenticated. **GET:** JSON **`{ messages, peer_profiles }`**. **`messages`:** list for the current user ( **`unread=true`** → only unread inbound). **`peer_profiles`:** map of peer user id → **`{ name, avatar_url }`** from RPC **`messaging_peer_profiles`** (only peers with an existing thread). **POST:** send a message; body includes **`receiver_id`**, **`content`**, optional **`subject`**, **`job_id`**, **`template_name`**. Recipient **`role`** is resolved via RPC **`user_role_for_id`**. **Role rules:** recruiter → job seeker only; job seeker → recruiter only. After insert, **`createNotificationForUser(receiver_id, "message", …)`** (service role) so the recipient gets a bell notification; **`data`** includes **`message_id`**, **`sender_id`**, **`job_id`**. |
| **/api/messages/mark-read** | POST | Body **`{ peer_id }`**. Marks inbound messages from that conversation partner as read for the current user. |
| **/api/messages/recipient-search** | GET | Query **`q`** (min 2 chars). Returns **`{ results: { id, name, email, role }[] }`** of opposite-role users matching name/email (compose **To** field). RPC **`search_message_recipients`** escapes **`%` / `_` / `\`** for **`ILIKE … ESCAPE '\'`** (migration **`20260404110000_messaging_search_hardening.sql`**); **`pg_trgm`** indexes on **`users.email`** / **`name`**. **Rate limit:** **`checkRecipientSearchRateLimit`** ( **`RECIPIENT_SEARCH_RATE_LIMIT_MAX`**, default **45**/min) via **`usage_logs.feature = message_recipient_search`**. **429** + **`Retry-After`**; server logs **`recipient_search_rate_limited`**. |
...
| Messages (job seeker) | `/messages` | Same **`MessagesInbox`** UI as recruiters: conversation list, thread + reply, compose. Query params: **`compose=1`**, **`receiver_id=`** (prefills compose to message a recruiter), optional **`peer=`** to open a thread. |
...
| Messages | `/recruiter/messages` | **`MessagesInbox`**: left column = conversations; main panel = selected thread + reply. Compose from URL: **`?compose=1&receiver_id=`** (and optional **`peer=`**). **`useMessages`** / **`useSendMessage`** → **`/api/messages`**; opening a thread calls **`POST /api/messages/mark-read`**. |
...
| **Sidebar** | Client; `navGroups` sections (Start here → Track & insights); **Messages** link to **`/messages`** (after Applications); mobile drawer, overlay, Escape to close, body scroll lock. |
...
| **RecruiterTopbar** | Recruiter header with **NotificationBell**; optional **company logo** badge (**`useRecruiterCompany`**) next to **`UserAvatar`**. |
...
| **NotificationBell** | Loads `/api/notifications`; Supabase Realtime **`INSERT`** on **`notifications`** with **`filter: user_id=eq.<current user>`** (avoids other users’ rows in the client); toast on insert; 60s poll fallback; mark read / mark all read. |
...
| Component | Role |
|-----------|------|
| **MessagesInbox** | Shared UI for **`/messages`** and **`/recruiter/messages`**: conversation list (**`UserAvatar`** + name from **`peer_profiles`**), thread + reply, compose; **`?compose=1&receiver_id=`** opens an existing thread when there is history (redirects to **`peer=`**); **`RecipientPicker`** searches opposite role by name/email via **`GET /api/messages/recipient-search`**; marks read via **`POST /api/messages/mark-read`** when a thread is opened. |
