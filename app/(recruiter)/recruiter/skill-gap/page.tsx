...
export default function SkillGapPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <h1 className="text-2xl font-bold text-text">AI Skill Gap Report</h1>
      ...
      <div className="flex gap-2">
        <button onClick={() => setMode("application")}
          className={`rounded-lg px-4 py-2 text-sm font-medium ${mode === "application" ? "bg-primary text-white" : "border border-gray-300 text-text"}`}>
          By Application ID
        </button>
        <button onClick={() => setMode("manual")}
          className={`rounded-lg px-4 py-2 text-sm font-medium ${mode === "manual" ? "bg-primary text-white" : "border border-gray-300 text-text"}`}>
          Manual Input
        </button>
      </div>

      <form onSubmit={handleAnalyze} className="space-y-4 rounded-xl border border-gray-200 bg-card p-5">
        {mode === "application" ? (
          <div>
      ...
      {result && (
        <div className="space-y-4">
          <div className="flex items-center gap-4 rounded-xl border border-gray-200 bg-card p-5">
