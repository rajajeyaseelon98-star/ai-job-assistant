    ...
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {templates.map((t) => (
            <div key={t.id} className="rounded-xl border border-gray-200 bg-card p-4">
              <div className="flex items-start justify-between">
                <div>
                  <div className="flex items-center gap-2">
                    <p className="text-sm font-medium text-text">{t.name}</p>
                    <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${TYPE_COLORS[t.template_type] || TYPE_COLORS.general}`}>
                      {TEMPLATE_TYPES.find((tt) => tt.value === t.template_type)?.label || t.template_type}
                    </span>
                  </div>
                  {t.subject && <p className="mt-0.5 text-xs text-text-muted">Subject: {t.subject}</p>}
                  <p className="mt-1 text-xs text-text-muted line-clamp-2">{t.content}</p>
                </div>
                <div className="flex gap-1">
                  <button onClick={() => startEdit(t)} className="rounded-lg p-1.5 text-text-muted hover:bg-blue-50 hover:text-blue-600">
                    <Edit3 className="h-4 w-4" />
                  </button>
                  <button onClick={() => handleDelete(t.id)} className="rounded-lg p-1.5 text-text-muted hover:bg-red-50 hover:text-red-600">
